...
public class ImageLoader {
	...
	public void displayImage(String uri, ImageAware imageAware, DisplayImageOptions options, ImageLoadingListener listener,
			ImageLoadingProgressListener progressListener, long time) {

		if (time > 0 && System.currentTimeMillis() - time > 0) {
			getDiskCache().remove(uri);
			getMemoryCache().remove(uri);
		}
		checkConfiguration();
		if (imageAware == null) {
	 ...
	 * @param options
	 * @param time
	 *            图片失效时间,毫秒时间 例如1415589600000
	 */
	public void displayImage(String uri, ImageView imageView, DisplayImageOptions options, long time) {
		displayImage(uri, new ImageViewAware(imageView), options, null, null, time);
	}
